...
You came to work in a big company as a Senior Python Developer.
  ...
  using the new style formatting, but it looks like it will take some
  time to persuade them: old habits die hard, especially in old-school
  programmers.
...
To show your colleagues that the new style formatting is not that
  ...
  function that will turn the old-style string formating s into a new
  one so that the following two strings have the same meaning:
...
def newStyleFormatting(s):
    return (re.sub(r'[+]{3}', '%', re.sub(r'%[bcdefgnosx]', '{}', re.sub(r'%%', '+++', s))))
